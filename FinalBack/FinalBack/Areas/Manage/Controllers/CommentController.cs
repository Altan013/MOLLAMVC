...
namespace FinalBack.Areas.Manage.Controllers
{
    ...
    public class CommentController : Controller
    {
        ...
        public IActionResult Blogcomments(/*int id,*/ int page = 1)
        {
            ...
            List<BlogComment> blogComments = _context.BlogComments.Include(c => c.AppUser).Include(c => c.Blog)/*.Where(c => c.ProductId == productid)*/.OrderByDescending(c => c.Id).Skip((page - 1) * 8).Take(8).ToList();
            ViewBag.CurrentPage = page;
            ViewBag.TotalPage = Math.Ceiling((decimal)_context.Comments.Include(c => c.Product)/*.Where(c => c.ProductId == productid)*/.Count() / 8);
            return View(blogComments);
        }
